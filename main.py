...
def run_web_server():
    # Render automatic PORT env variable deta hai, agar na mile toh default 10000 ya 8080 par chalega
    port = int(os.environ.get("PORT", 10000))
    
    # Hum dono ports ko handle karne ke liye safe check laga rahe hain
    try:
...
async def main():
    ...
    # Background dummy server setup for render port requirement
    threading.Thread(target=run_web_server, daemon=True).start()
    
    await app.start()
    # Continuous active state loop
