...
# ... baki aapka poora animation aur predict wala code bilkul same rahega ...
import asyncio
import random
import os
...
# --- DIRECT EVENT LOOP MANAGEMENT FOR PYTHON 3.14 ---
async def start_bot():
    print("🔥 SPYYT ESCROW FLIP BOT STARTED 🔥")
    # Port web server start karna
    threading.Thread(target=run_web_server, daemon=True).start()
    
    # Client initialize aur start karna bina loop error ke
    await app.start()
    
    # Bot ko block/chalu rakhne ke liye infinite loop
    try:
...
if __name__ == "__main__":
    # Naye system me naya loop khud create kar rahe hain
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(start_bot())
# --- SABSE NEECHE JO MAIN BLOCK HAI USE AISE RAKHO ---
async def main():
    print("🔥 SPYYT ESCROW FLIP BOT STARTED 🔥")
    threading.Thread(target=run_web_server, daemon=True).start()
    await app.start()
